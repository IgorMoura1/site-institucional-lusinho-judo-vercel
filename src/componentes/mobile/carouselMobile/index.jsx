        ...
        <div className="carouselContainerMobile">
            <Slider ref={sliderRef} {...settings}>
                {items.map((item, index) => (
                    <div key={index} className={`carouselItemMobile ${index === activeIndex ? 'active' : 'sideActive'}`}>
                        <div className="carouselItemMobileWrapperMobile">
                            <h2>{item.title}</h2>
                            <img src={item.image} alt={item.title} className="carouselImageMobile" />
                            <p>{item.description}</p>
                        </div>
                    </div>
                ))}
            </Slider>
            <div className="carouselNavigationMobile">
                <button className="navButtonMobile left" onClick={() => sliderRef.current.slickPrev()}></button>
                <button className="navButtonMobile right" onClick={() => sliderRef.current.slickNext()}></button>
            </div>
        </div>
    );
};
